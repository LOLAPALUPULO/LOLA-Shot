...
const GameEngine: React.FC<GameEngineProps> = ({ onGameOver, gameState, setGameState }) => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Enemies - DYNAMIC DIFFICULTY SYSTEM
      // Calculate dynamic spawn rate: Start at 120 frames, decrease by 1.5 per point, floor at 30 frames (0.5s)
      const currentSpawnRate = Math.max(30, GAME_CONFIG.SPAWN_RATES.ENEMY - (s.score * 1.5));
      
      if (s.frameCount > s.nextEnemySpawn) {
        s.nextEnemySpawn = s.frameCount + currentSpawnRate;

        // Calculate Wave Density (number of enemies per spawn)
        // Score 0-25: 1 enemy
        // Score 25-60: 1-2 enemies
        // Score 60+: 2-3 enemies
        let maxDensity = 1;
        if (s.score >= 25) maxDensity = 2;
        if (s.score >= 60) maxDensity = 3;
        // Increase challenge if player is powered up
        if (s.shotPower >= 3) maxDensity = Math.min(maxDensity + 1, 4);

        const enemiesToSpawn = Math.floor(Math.random() * maxDensity) + 1;
        ...
        for(let i=0; i<enemiesToSpawn; i++) {
            ...
            // Probabilities shift as score increases
            ...
            // Late Game Health Scaling (Score > 80)
            if (s.score > 80 && Math.random() > 0.5) {
              health += 1;
            }

            s.enemies.push({
              id: Math.random().toString(),
              x: gameWidth + (i * 60) + (Math.random() * 50), // Stagger X slightly
      ...
      // Bonus (Lupulo) - Every 10 points (Changed from 15)
      if (s.score >= 10 && s.score % 10 === 0 && s.score > s.lastBonusScore && s.bonuses.length === 0) {
        s.bonuses.push({
          id: Math.random().toString(),
          x: gameWidth,
